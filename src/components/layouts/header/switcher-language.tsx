...
export const SwitcherLanguage = ({ defaultValue }: ISwitcherLanguageProps) => {
  ...
  return (
    <div className="flex flex-col gap-y-1">
      <button
        type="button"
        className={`hover:cursor-pointer transition duration-300 rounded-full p-0.5 ${
          defaultValue === 'pt' && 'transform scale-125'
        }`}
        title={defaultValue === 'pt' ? 'Português' : 'Portuguese'}
        onClick={() => handleChangeLanguage('pt')}
      >
        <Image src="/icons/pt-br.webp" alt="pt-br" width={16} height={16} />
      </button>
      <button
        type="button"
        className={`hover:cursor-pointer transition duration-300 rounded-full p-0.5 ${
      ...
      >
        <Image src="/icons/en-us.webp" alt="en-us" width={16} height={16} />
      </button>
    </div>
  );
};
